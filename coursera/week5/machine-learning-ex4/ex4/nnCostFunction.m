...
for i = 1:m,
  inner = 0;
  for k = 1:num_labels,
    term1 = (-y_vals(i,k))*log(h2(i,k));
    term2 = (1 - y_vals(i,k))*log(1 - h2(i,k));
    inner = inner + (term1 - term2);
  end;
  outer = outer + inner;
end;
...
for i = 1:size(Theta1, 1),
  regterm1 = regterm1 + sumsq(Theta1(i,2:end));
end;

regterm2 = 0;
for i = 1:size(Theta2, 1),
  regterm2 = regterm2 + sumsq(Theta2(i,2:end));
end;
regterm = (lambda * (regterm1 + regterm2)) / (2*m);
